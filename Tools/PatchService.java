...
public class PatchService {
     ...
     */
    public void createDifFiles(final Props props, final String referencePath,
        ...
        if (referencePath != null) {
            // this is not needed by Lemmini, but to create the DIF files (and
            // CRCs)
            if (DO_CREATE_CRC) {
                // create crc.ini
                Extract.out("\nCreate CRC ini");
            ...
            createPatches(props, patchPath, referencePath, destinationPath,
                    createdFiles);
        }
    }
     ...
     *                          files.
     */
    public void patchAllFiles(final String destinationPath)
            throws ExtractException {
        // step eight: use patch.ini to extract/patch all files
        // read patch.ini file
        final Props pprops = new Props();
        final URL fnp = Extract
                .findFile(patchPath + PATCH_INI_NAME/* , this */); // if it's in
                                                                   // the JAR or
                                                                   // local
                                                                   // directory

        if (!pprops.load(fnp)) {
            throw new ExtractException("File " + PATCH_INI_NAME
        ...
        for (int i = 0; true; i++) {
            ...
            final URL fnc = Extract
                    .findFile(patchPath + fnDecorated /* , pprops */);

            try {
                copyFile(fnc, destinationPath + copy[0]);
            } catch (final Exception ex) {
                throw new ExtractException(
                        "Copying " + patchPath + getFileName(copy[0]) + " to "
                                + destinationPath + copy[0] + " failed");
            }
    ...
    private void createCRCs(final String rPath, final String sDir,
        ...
        if (files == null) {
            throw new ExtractException(
                    "Path " + rootPath + " doesn't exist or IO error occured.");
        }
        ...
        outerLoop: for (int i = 0; i < files.length; i++) {
            ...
            try {
                ...
                fCRCList.write("crc_" + (Integer.toString(crcNo++)) + " = "
                        + out + "\n");
            } catch (final Exception ex) {
     ...
     * @throws ExtractException if an extraction error occurs.
     */
    private void createPatches(final String sPath, final String dPath,
     ...
     * @param fPatchList   FileWriter to create patch.ini
     * @param fSource      a File representation of the folder containing the
     *                     source files.
     * @param destPath     the folder to which the patch files are to be
     *                     written.
     * @param createdFiles Map of files created.
     * @throws ExtractException if an extraction error occurs.
     */
    private void patchFiles(final String sDir, final FileWriter fPatchList,
            final File fSource, final String destPath,
            final Map<String, Object> createdFiles) throws ExtractException {
        String out;
        final File[] files = fSource.listFiles();
        ...
        outerLoop: for (int i = 0; i < files.length; i++) {
            ...
            fnPatch = patchPath + subDirDecorated
                    + (fnPatch.substring(0, pos) + ".dif").toLowerCase();
            ...
            try {
                ...
                if (!fileExists) {
                    copyMissingFilesToPatchDirectory(fPatchList, files,
                            subDirDecorated, i, src);
     ...
     */
    private boolean targetFileExists(final String fnOut,
            final Map<String, Object> createdFiles) {
        boolean fileExists;
    ...
    /**
     * Given that the specified file does not exist, mark it as missing and
     * needing to be extracted from the JAR file. Then, copy missing files to
     * the patch dir.
     *
     * @param fPatchList FileWriter to create patch.ini.
    ...
    private String copyMissingFilesToPatchDirectory(final FileWriter fPatchList,
        ...
        crc.update(src);
        out = subDir + files[filesIndex].getName() + ", 0x"
                + Integer.toHexString((int) crc.getValue());
        fPatchList.write("extract_" + (Integer.toString(extractNo++)) + " = "
                + out + "\n");
        // copy missing files to patch dir
        Extract.copyFile(fnIn,
                patchPath + subDirDecorated + files[filesIndex].getName());
        return out;
    }
     ...
     *
     * @param fPatchList FileWriter to create patch.ini.
     * @param out        String with full path and filename + CRC for patch
     *                   file.
     * @param fnPatch    Path and filename for .DIF file.
     * @param trg        Bytes read from existing destination file.
     * @param patch      Buffer with differences between source and target
     *                   buffer.
     * @throws IOException      if an I/O error occurs.
     * @throws DiffException    if a Diff error occurs.
     * @throws ExtractException if an extraction error occurs.
     */
    private void writePatchFileIfPatchExists(final FileWriter fPatchList,
            final String out, final String fnPatch, final byte[] trg,
            final byte[] patch)
            throws IOException, DiffException, ExtractException {
        if (patch == null) {
            // Extract.out("src and trg are identical");
            fPatchList.write("check_" + (Integer.toString(checkNo++)) + " = "
                    + out + "\n");
        } else {
            ...
            fPatchList.write("patch_" + (Integer.toString(patchNo++)) + " = "
                    + out + "\n");
        }
    }
     ...
     */
    private void patchFiles(final Props pprops, final String destinationPath)
            throws ExtractException {
        // patch
        Extract.out("\nPatch files");
        ...
        for (int i = 0; true; i++) {
            ...
            if (urlDif == null) {
                throw new ExtractException("Patching of file " + destinationPath
                        + ppath[0] + " failed.\n");
            }
    ...
    private void writeFile(final String fname, final byte[] buf)
        ...
        try (FileOutputStream f = new FileOutputStream(fname);) {
            ...
            throw new ExtractException(
                    "IO exception while writing file " + fname);
        }
    }
