...
public class Sound {
	...
	public Sound(final int snum, final int pitchID) throws ResourceException {
		...
		try {
			for (int i = 0; i < sampleNum; i++) {
				...
				try (final AudioInputStream f = AudioSystem.getAudioInputStream(fs.toURI().toURL())) {
		...
		if (pitchID >= 0) {
			...
			// note that bit size (8) and channels (1) have to be the same for all pitched
			// buffers
		...
		final ArrayList<Mixer> mix = new ArrayList<Mixer>();
	 ...
	 * @param format AudioFormat of source sample (only unsigned 8bit PCM or signed
	 *               16bit PCM supported)
	 * @return sample converted to default format (16bit signed PCM 22050Hz) stored
	 *         in byte array
	 */
	public synchronized byte[] convertToDefault(final byte buffer[], final AudioFormat format) {
		...
		if ((format.getFrameRate() == DEFAULT_FREQUENCY) && (format.getFrameSize() == 2) && !format.isBigEndian()
				&& (format.getEncoding() == AudioFormat.Encoding.PCM_SIGNED))
			return buffer;

		final boolean from8bit = (format.getFrameSize() == 1);
		final boolean convertEndian = ((format.getFrameSize() == 2) && format.isBigEndian());

		// sample up low frequency files to a DEFAULT_FREQUENCY to work around sound bug
		// in JDK6
		final double scale = DEFAULT_FREQUENCY / format.getSampleRate();
		final int sampleNumSrc = buffer.length / (from8bit ? 1 : 2);
		final int sampleNumTrg = (int) (buffer.length * scale) / (from8bit ? 1 : 2); // length of target buffer in
																						// samples
	...
	public synchronized byte[] createPitched(final int idx, final int pitch) {
		...
		// then increase the sample rate virtually by creating a buffer which contains
		// only
		// every Nth sample

		if (format[idx].getFrameSize() != 2) // only 16bit supported
			return null;
		final double scale = pitchFormat.getSampleRate() / format[idx].getSampleRate();
		double dpitch = (1.0 + ((pitch - 1) * 0.0204));
		if (dpitch < 1.0)
			dpitch = 1.0;
		final double fact = dpitch / scale;
		final int len = (int) (soundBuffer[idx].length / (2 * fact)); // length of target buffer in samples
