...
public class Stencil {
    /*
     * Each pixel is represented by a 32bit value in the stencil. The lower part
     * is used for a bitmask that contains stencil properties. The higher part
     * (val << ID_SHIFT_VAL) can contain an identifier for a special trap, exit
     * etc. Note: the lowe part is a bitmask, mainly to save space, but also
     * since some combinations of properties are possible. Yet of course not all
     * combinations are possible or make sense.
     */
    ...
    /**
     * stair build by a builder - note that this is just an additional attribute
     * - brick is also needed to walk on it.
     */
    ...
    public static final int MSK_ERASE = ~(Stencil.MSK_WALK_ON
            | Stencil.MSK_STAIR | Stencil.MSK_NO_DIG);
    ...
    public static final int MSK_TRAP = MSK_TRAP_DROWN | MSK_TRAP_REPLACE
            | MSK_TRAP_DIE;
    /** the level exit. */
