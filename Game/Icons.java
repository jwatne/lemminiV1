...
public final class Icons {
    ...
    /**
     * Number of icon sprites.
     */
    private static final int NUM_ICON_SPRITES = 15;
    ...
    public static void init(final Component cmp) throws ResourceException {
        iconImg = ToolBox.createImage(WIDTH * (1 + LAST_DRAWN), HEIGHT,
                Transparency.OPAQUE);
        iconGfx = iconImg.createGraphics();
        MediaTracker tracker = new MediaTracker(cmp);
        icons = new Sprite[NUM_ICON_SPRITES];

        for (int i = 0; i < (NUM_ICON_SPRITES - 1); i++) {
            Image sourceImg = Core.loadImage(tracker,
                    "misc/icon_" + i + ".gif");
            ...
            icons[i] = new Sprite(sourceImg,
                    (i == Type.EMPTY.ordinal()) ? 1 : 2);
     ...
     *
     * @param type
     * @return pressed state of the Icon.
     */
    static boolean isPressed(final Type type) {
    ...
    public static void press(final Type type) {
        ...
        case PAUSE:
        case FFWD:
            icons[idx].setFrameIdx((icons[idx].getFrameIdx() == 0) ? 1 : 0);
            // toggle
