...
public class Level {
	...
	/**
	 * array of special sprite objects - with transparency, drawn above background
	 * image
	 */
	...
	void loadLevel(final String fname) throws ResourceException, LemmException {
		...
		final String fnames = Core.findResource("styles/" + strStyle + "/" + strStyle + ".ini");
		props = new Props();
	...
	Stencil paintLevel(final BufferedImage bgImage, final Component cmp, final Stencil s) {
		...
		for (int n = 0; n < terrain.size(); n++) {
			...
			final int tx = t.xPos;
			final int ty = t.yPos;
			final boolean upsideDown = (t.modifier & Terrain.MODE_UPSIDE_DOWN) != 0;
			final boolean overwrite = (t.modifier & Terrain.MODE_NO_OVERWRITE) == 0;
		...
		final ArrayList<SpriteObject> oCombined = new ArrayList<SpriteObject>(64);
		final ArrayList<SpriteObject> oBehind = new ArrayList<SpriteObject>(64);
		final ArrayList<SpriteObject> oFront = new ArrayList<SpriteObject>(4);
		final ArrayList<Entry> entry = new ArrayList<Entry>(4);
		AffineTransform tx;
		for (int n = 0; n < objects.size(); n++) {
			try {
				final LvlObject o = objects.get(n);
				// if (sprObjAvailable[o.id].animMode != Sprite.ANIM_NONE) {
				final SpriteObject spr = new SpriteObject(sprObjAvailable[o.id]);
				spr.setX(o.xPos);
				spr.setY(o.yPos);
				...
				if (spr.getType() == SpriteObject.Type.ENTRY && !o.upsideDown) {
					final Entry e = new Entry(o.xPos + spr.getWidth() / 2, o.yPos);
					e.id = oCombined.size();
					entry.add(e);
				...
				if (!o.upsideDown) {
					for (int y = 0; y < spr.getHeight(); y++) {
						...
						for (int x = 0; x < spr.getWidth(); x++) {
							...
							if (/* paint && */ spr.getType() != SpriteObject.Type.ENTRY
	 ...
	 */
	public BufferedImage createMiniMap(final BufferedImage image, final BufferedImage bgImage, final int scaleX,
			final int scaleY, final boolean tint) {
	...
	public int[] getParticleCol() {
		// Return clone to avoid malicious code vulnerability of potentially exposing
	...
	/**
	 * Get number of Lemmings to rescue : should be less than or equal to number of
	 * Lemmings.
	 * 
	...
	/**
	 * Get number of Lemmings in this level (maximum 0x0072 = 114 in original LVL
	 * format).
	 * 
